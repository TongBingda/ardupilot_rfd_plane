...
static const ap_message STREAM_POSITION_msgs[] = {
    ...
    MSG_LOCAL_POSITION,
    // RFD_TELEM==ENABLED
    // LEADER_STATUS message should be taken as location message
    MSG_LEADER_STATUS,
};
...
// RFD_TELEM==ENABLED
    case MAVLINK_MSG_ID_LEADER_STATUS: // MAVLINK_MSG_ID_LEADER_STATUS for arduplane only, mav id = 235
    {
        handle_leader_status(msg);
        break;
    }
...
// RFD_TELEM==ENABLED
void GCS_MAVLINK_Plane::handle_leader_status(const mavlink_message_t &msg)
{
    mavlink_leader_status_t m;
    mavlink_msg_leader_status_decode(&msg, &m);
    gcs().send_text(MAV_SEVERITY_INFO, "Leader status: sysid=%d, latitude=%f, longitude=%f, altitude=%f, heading=%f, airspeed=%f, groundspeed=%f, throttle=%f, roll=%f, pitch=%f, yaw=%f, reserved1=%f ,reserved2=%d, command=%d.", msg.sysid, m.latitude, m.longitude, m.altitude, m.heading, m.airspeed, m.groundspeed, m.throttle, m.roll, m.pitch, m.yaw, m.reserved1, m.reserved2, m.command);
    AP_Notify::play_tune("MFT100L8>B");
}

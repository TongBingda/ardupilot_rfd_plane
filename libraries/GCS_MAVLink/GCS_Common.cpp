...
// RFD_TELEM==ENABLED
void GCS_MAVLINK::send_leader_status()
{
    const AP_AHRS &ahrs = AP::ahrs();
    ahrs.get_position(global_position_current_loc);
    mavlink_msg_leader_status_send(
        chan, 
        AP_HAL::millis(),
...
bool GCS_MAVLINK::try_send_message(const enum ap_message id)
{
    ...
    case MSG_LEADER_STATUS:
        // RFD_TELEM==ENABLED
        if (mavlink_system.sysid == 1) // only leader uav (sysid=1) send its status, follower uav send nothing
        {
            send_leader_status();
        }
        break;
